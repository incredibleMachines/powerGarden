...
public class CosmParser {
	...
	public CosmParser(JSONObject root) throws JSONException {
		data = root;
		parseLocation(root);
		parseStreams(root);
	}
...
//			System.out.println("CosmParser: parseLocation()");
	...
	private void parseFeedTags() throws JSONException {
		if (data.has("tags")) {
	...
	public int getDataStreamCount() throws JSONException {
	...
	private void parseStreams(JSONObject data) throws JSONException {
//		if (myParent.verbose)
//			System.out.println("CosmParser: parseStreams()\n\n\n");
		Log.d("parseStreams", "---parsing streams---");
		Log.d("parseStreams", data.toString());
		...
		if (data.has("datastreams")) {
			...
			for (int i = 0; i < datastreamCount; i++) {
				...
				try {
					...
					JSONArray srows = ((JSONObject) stream).names();
					//String[] srows = stream.get
					for (int j = 0; j < srows.length(); j++) {
						String row = srows.getString(j);
						if (row.equals("at"))
	...
	public String getLastUpdateTimestamp(int id) throws JSONException {
		if (data.has("datastreams")) {
	...
	// --- get stream Id Name
	public String getStreamId(int id) throws JSONException {
		if (data.has("datastreams")) {
	...
	// --- get stream max val
	public float getStreamMaxVal(int id) throws JSONException {
		if (data.has("datastreams")) {
			if (id < data.getJSONArray("datastreams").length()) {
				float data = 0.0f;
	...
	// --- get stream min val
	public float getStreamMinVal(int id) throws JSONException {
		if (data.has("datastreams")) {
			if (id < data.getJSONArray("datastreams").length()) {
				float data = 0.0f;
