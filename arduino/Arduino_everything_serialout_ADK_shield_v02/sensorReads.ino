...
void readVibrationSensors(){
  if(enable_vib_sensor){
    int vibVal[3] = {
      0,0,0      };

    vibVal[0] = analogRead(vibSensor0);
    vibVal[1] = analogRead(vibSensor1);
    vibVal[2] = analogRead(vibSensor2);

    if(debug==true){
      Serial.print("Vib 1: ");
      Serial.print(vibVal[0]);
      Serial.print("   Vib 2: ");
      Serial.print(vibVal[1]);
      Serial.print("   Vib 3: ");
      Serial.print(vibVal[2]);
      Serial.print("\t");
    }
    else{
      //real serial write  
      sprintf(outgoing, "V,%d,%d,%d",vibVal[0],vibVal[1],vibVal[2]);
